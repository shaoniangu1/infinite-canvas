...
const generationOptionsSchema = z.object({
    ...
    videoCharacterOrientation: z.string().optional(),
    videoBackgroundSource: z.string().optional(),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    workbench_image_get_config: "读取生图工作台的当前参数和可选项（可用模型、画质、清晰度、宽高比、张数范围），在调用 workbench_image_generate 前先了解可选值。",
    workbench_image_generate: "在生图工作台填入提示词并按需设置 model、quality、resolution、size（如 1:1）、count，run 默认 true 会自动点击生成按钮。会自动跳转生图工作台，工具返回代表已提交。",
