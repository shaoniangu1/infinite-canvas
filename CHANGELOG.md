...
+ [新增] 新增面向 AI 编码助手的新模型 / 新渠道接入指引，统一 Model Profile、Provider Adapter 和调用脚本的职责及验收要求。
+ [调整] 合并上游 `v0.9.0`，保留画布插件系统、左侧资产面板、批量导出、节点名称显示优化和模型自定义调用脚本。
+ [新增] 模型渠道支持内置 Alibbit / KIE Provider Adapter，并与上游按模型配置的自定义调用脚本共存。
+ [新增] KIE 视频模型支持 Kling 2.6 / 3.0 motion-control 和 Seedance 2 / Fast / Mini，生成配置按模型能力显示并校验参数。
+ [新增] KIE 新增 Gemini Omni Video 核心视频工作流，支持提示词、参考图、单个参考视频及裁剪、时长、比例、分辨率和 Seed 参数。
+ [新增] 新增第三方“视频音轨工具”插件及 `ctx.media` SDK 能力，可在浏览器本地分离视频音轨并将音频重新合并到生成视频。
+ [优化] 图像设置统一为画质、清晰度、比例和生成数量，视频设置按模型能力渲染；画布设置浮层固定在节点右侧并支持内部滚动。
+ [修复] 修复 Alibbit 实际模型映射、KIE 异步任务错误与结果解析、Kling 空提示词重试、默认生成数量和文本节点滚轮交互问题。
+ [修复] 修复第三方视频音轨工具节点无法拖动及处理资源加载时报 `Invalid URL` 的问题。
+ [优化] 自定义视频调用脚本新增参考视频和参考音频输入，支持完整媒体生成工作流。
