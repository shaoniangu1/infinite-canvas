...
import { isBuiltinModelProvider } from "@/services/ai/model-profiles";
import { syncAppDataToWebdav, type AppSyncDomainKey, type AppSyncProgressEvent } from "@/services/app-sync";
import { testWebdavConnection, WEBDAV_MANIFEST_FILE_NAME } from "@/services/webdav-sync";
import { audioFormatOptions, audioVoiceOptions, normalizeAudioSpeedValue } from "@/lib/audio-generation";
import { createModelChannel, guessCapability, modelOptionsFromChannels, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore, type AiConfig, type ApiCallFormat, type ChannelModel, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const refreshAllModels = async () => {
        const channels = config.channels.filter((channel) => channel.baseUrl.trim() && (isBuiltinModelProvider(channel.apiFormat) || channel.apiKey.trim()));
        if (!channels.length) {
        ...
        try {
            const entries = await Promise.all(channels.map(async (channel) => [channel.id, await fetchChannelModels(channel)] as const));
            const modelsByChannel = new Map(entries);
            updateChannels(config.channels.map((channel) => ({ ...channel, models: mergeFetchedModels(channel.models, modelsByChannel.get(channel.id)) })));
                            ...
                            <div>
                                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                    ...
                                    <div className="flex shrink-0 gap-2">
                                        <Button icon={<RefreshCw className="size-4" />} loading={refreshingModels} onClick={() => void refreshAllModels()}>
                                            拉取全部
                                        </Button>
...
function mergeFetchedModels(current: ChannelModel[], fetched?: string[]) {
    if (!fetched) return current;
    const existing = new Map(current.map((model) => [model.name, model]));
    return fetched.map((name) => existing.get(name) || { name, capability: guessCapability(name) });
}
