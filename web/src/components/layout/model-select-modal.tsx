...
export function ModelSelectModal({ open, channel, selectedNames, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedNames: string[]; onConfirm: (names: string[]) => void; onClose: () => void }) {
    ...
    const fetchModels = async () => {
        if (!channel) return;
        if (!channel.baseUrl.trim() || (!isBuiltinModelProvider(channel.apiFormat) && !channel.apiKey.trim())) {
            message.error(isBuiltinModelProvider(channel.apiFormat) ? "请先填写接口地址" : "请先填写接口地址和 API Key");
            return;
        }
        setLoading(true);
        try {
